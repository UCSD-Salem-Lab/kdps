...
system.time({
  ...
  subject_to_remove_list = vector()
  while(dim(kinship)[1] > 0){
    relationship = table(c(
      kinship[["fid1_iid1"]],
      kinship[["fid2_iid2"]]
    ))
    
    max_count = max(relationship) - fuzziness
    
    subject_to_remove = (tibble(
      fid_iid = names(relationship[relationship >= max_count])
    ) %>%
      left_join(pheno, by = "fid_iid") %>%
      arrange(wt))[["fid_iid"]][1]
    
    # relationship = tibble(
    #   fid_iid = names(relationship),
    #   count = as.vector(relationship)
    # ) %>%
    #   arrange(desc(count))
    # 
    # max_count = max(relationship[["count"]]) - fuzziness
    # 
    # subject_to_remove = (relationship %>%
